...
3. Configure the application:
   ...
   Edit the [`.env`](./.env) file to provide the runtime configuration used by [`env.sh`](./env.sh):
   ```
...
- Sign in with Google and GitHub OAuth integrations
...
- `GOOGLE_CLIENT_ID`: Google OAuth application client ID used by the frontend Google sign-in flow
- `GITHUB_CLIENT_ID`: GitHub OAuth application client ID used to build the GitHub authorization URL in the frontend
- `EMAIL_VERIFICATION_RESEND_COOLDOWN_SECONDS`: Cooldown in seconds before users can request another email verification message

These values are injected at runtime through `env-config.js`, which is generated by [`env.sh`](./env.sh) and exposed in the app as `window._env_`.

Additional development-server configuration can be set in [`vite.config.ts`](./vite.config.ts).
